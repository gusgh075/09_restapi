package org.ho.springsecurity.auth.service;

import lombok.RequiredArgsConstructor;
import org.ho.springsecurity.auth.dto.LoginRequest;
import org.ho.springsecurity.auth.repository.AuthRepository;
import org.ho.springsecurity.auth.dto.TokenResponse;
import org.ho.springsecurity.command.entity.User;
import org.ho.springsecurity.command.repository.UserRepository;
import org.ho.springsecurity.jwt.JwtTokenProvider;
...
public class AuthService {
   ...
   * access token이 만료되었을 때 자동으로 이 서비스를 호출할 것임 */
  public TokenResponse refreshToken(String providedRefreshToken) {
    ...
    // RefreshToken Entity create(for save)
    RefreshToken tokenEntity = RefreshToken.builder()
        .username(username)
        .token(refreshToken)
        .expiryDate(
