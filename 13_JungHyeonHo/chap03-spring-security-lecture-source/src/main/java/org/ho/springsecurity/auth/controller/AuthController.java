package org.ho.springsecurity.auth.controller;

import lombok.RequiredArgsConstructor;
import org.antlr.v4.runtime.Token;
import org.ho.springsecurity.auth.dto.LoginRequest;
import org.ho.springsecurity.auth.service.AuthService;
...
public class AuthController {
  ...
  public ResponseEntity<ApiResponse<TokenResponse>> refreshToken(
     ...
     * -> 현재 사이트를 계속 이용중이라면 token을 계속 갱신해줌
     * */
  ...
  public ResponseEntity<ApiResponse<Void>> logout(
    ...
    // refreshToken이 존재할 경우 == login status
    if (refreshToken != null) {
      authService.logout(refreshToken); // DB refresh token 삭제
    }
